#!/bin/bash
# 微信服务号 AI 新闻简报 - Kimi 2.5 封面图生成脚本
# 使用月之暗面 Kimi 2.5 模型生成专业封面图

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT_DIR="$SCRIPT_DIR/../output/covers"
LOG_FILE="$SCRIPT_DIR/../logs/kimi-cover-generation.log"
...
log() {
    echo -e "[$(date '+%Y-%m-%d %H:%M:%S')] ${BLUE}[Kimi 2.5 封面]${NC} $1" | tee -a "$LOG_FILE"
}
...
generate_cover() {
    ...
    log "输出：$output_file"
    log "时段：$period"
    log "模型：Kimi 2.5 (Moonshot AI)"
    
    # 检查 API 凭证
    if [ -z "$KIMI_API_KEY" ]; then
        warn "KIMI_API_KEY 未设置"
        log "请设置环境变量:"
        log "  export KIMI_API_KEY='your_api_key'"
        log ""
        log "获取 API Key: https://platform.moonshot.cn/console/api-keys"
        
        # 创建占位文件
        echo "封面图占位 - Kimi 2.5 - 标题：$title - 时段：$period - 需配置 KIMI_API_KEY" > "${output_file}.txt"
        return 1
    fi
...
请生成符合上述要求的专业封面图。"
    ...
    log "生成 prompt..."
    log "使用模型：Kimi 2.5 (Moonshot AI)"
    
    # 生成时间戳文件名
    local timestamp=$(date '+%Y-%m-%d-%H-%M-%S')
    local temp_filename="cover-${timestamp}-kimi.png"
    ...
    # Kimi API 端点（图像生成）
    local api_url="https://api.moonshot.cn/v1/images/generations"
...
# 显示使用说明
show_usage() {
    echo "用法：generate-cover-kimi.sh <标题> <输出文件> <时段 (00|06|12|18)>"
    echo ""
    echo "环境变量:"
    echo "  KIMI_API_KEY    Kimi API Key (从 https://platform.moonshot.cn 获取)"
    echo ""
    echo "示例:"
    echo "  export KIMI_API_KEY='sk-xxxxxxxxx'"
    echo "  ./scripts/generate-cover-kimi.sh 'AI News' 'cover.png' '12'"
    echo ""
    echo "时段说明:"
    echo "  00 - 午夜场 (00:00)"
    echo "  06 - 晨间场 (06:00)"
